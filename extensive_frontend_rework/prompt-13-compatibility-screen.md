# Prompt 13 — CompatibilityScreen MD3 Rewrite

## Task

Rewrite `mobile/src/screens/CompatibilityScreen.tsx` using Material Design 3 Paper components. All existing logic — compatibility calculation, save-to-storage, breakdown toggle — must be preserved exactly.

---

## Context

**Current file:** `mobile/src/screens/CompatibilityScreen.tsx`
...
Preserve `getRatingLabel()`, `BREAKDOWN_LABELS`, all state (`partnerName`, `partnerDate`, `result`, `showBreakdown`, `dateError`), and all handlers (`handleCheck`, `handleSave`).

---

## Required Implementation

Replace `CompatibilityScreen.tsx` completely with:
...
- `Banner` shows when no birth date is set.
- Score shows as `displayMedium` text in `primaryContainer` card.
- Strengths are `secondaryContainer` Chips.
- Cautions are `errorContainer` Chips.
- `List.Accordion` expands to show 8 Ashtakoot rows.
- "Save This Check" writes to storage.
- No hardcoded colors.
