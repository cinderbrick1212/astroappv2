# Prompt 09 — HomeScreen MD3 Rewrite

## Task

Rewrite `mobile/src/screens/HomeScreen.tsx` using Material Design 3 Paper components. All existing logic — greeting calculation, focus area derivation, navigation — must be preserved exactly.

---

## Context

**Current file:** `mobile/src/screens/HomeScreen.tsx`

**All preserved logic:**
- `getGreeting()` helper
- `FOCUS_AREAS`, `FOCUS_MESSAGES` constants
- `QUICK_ACTIONS` constant
- `useAuth()`, `useNavigation()`, `analytics.screenView()` calls

---

## Required Implementation

Replace `HomeScreen.tsx` completely with:
...
- `FOCUS_AREAS` now uses Material Community Icon names instead of emoji — Copilot must not revert to emoji strings.
- `QUICK_ACTIONS` also uses icon names.
- `FAB` with `variant="secondary"` and `size="medium"` creates the MD3 "Medium FAB" style.
- If `FAB` does not accept `label` in the current `react-native-paper` version, use `FAB.Group` or fall back to an `elevated` `Card` with a `List.Item` inside.

---

## Validation

- Greeting card uses `primaryContainer` background.
- Focus card shows icon and message.
- 4 FABs render in a 2×2 grid; each navigates correctly.
- Service cards navigate to `AskQuestion` and `BookCall`.
- No hardcoded colors.
